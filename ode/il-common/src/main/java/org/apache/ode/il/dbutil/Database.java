...
public class Database {
    ...
    private void initDataSource() throws DatabaseConfigException {
        ...
        case EMBEDDED:
            //initEmbeddedDb();
            initEmbeddedH2Db(); //Moved embedded db to H2
            break;
        case INTERNAL:
            initInternalDb();
            break;
    ...
    private void initInternalDb(String url, String driverClass, String username,String password) throws DatabaseConfigException {
        ...
        try {
            mcf.setDriver(driverClass.trim());
            mcf.setConnectionURL(url.trim());
            if (username != null) {
                mcf.setUserName(username.trim());
            }
            if (password != null) {
     ...
     */
    private void initEmbeddedH2Db() throws DatabaseConfigException {
        String db = _odeConfig.getDbEmbeddedName();
        String url = "jdbc:h2:" + _workRoot + "/" + db + ";MVCC=TRUE;DB_CLOSE_ON_EXIT=FALSE";
    ...
    private <T> T lookupInJndi(String objName) throws Exception {
		...
		if(_odeConfig.getProperties().getProperty("ode-axis2.jndi.context.factory") != null){
		    jndiEnv.setProperty(Context.INITIAL_CONTEXT_FACTORY,
		    _odeConfig.getProperties().getProperty("ode-axis2.jndi.context.factory").trim());
		}
		if(_odeConfig.getProperties().getProperty("ode-axis2.jndi.provider.url") != null){
		    jndiEnv.setProperty(Context.PROVIDER_URL,
		    _odeConfig.getProperties().getProperty("ode-axis2.jndi.provider.url").trim());
		}
