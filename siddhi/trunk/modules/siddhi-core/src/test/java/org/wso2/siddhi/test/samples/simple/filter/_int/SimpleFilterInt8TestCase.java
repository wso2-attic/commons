...
public class SimpleFilterInt8TestCase extends SimpleFilterBaseTestCase {

    private final Logger log = Logger.getLogger(this.getClass());

    protected SimpleCondition createAPICondition(QueryFactory qf) {
        return qf.condition("(4+1)*3", EQUAL, "CSEStream.price+3*2");
    }

    @Override
    protected String createQueryCondition() {
        return "(4+1)*3 == CSEStream.price+3*2";
    }

    protected void assertEvent(Event event) {
        Assert.assertEquals(event.getNthAttribute(0), 9);
    }
