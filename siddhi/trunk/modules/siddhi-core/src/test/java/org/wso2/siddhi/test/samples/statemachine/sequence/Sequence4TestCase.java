...
public class Sequence4TestCase {

    private static final Logger log = Logger.getLogger(Sequence4TestCase.class);
    private volatile int i;
    private volatile boolean eventCaptured;

    @Before
    public void info() {
        log.debug("-----Query processed: Testing replete sequence with star staring with non accepting event-----");
        i = 0;
        eventCaptured = false;
    }
  ...
  @Test
    ...
    private void sendEvents(InputHandler inputHandlerStockQuoteStream) {
        ...
        inputHandlerStockQuoteStream.sendEvent(new EventImpl("StockQuoteStream", new Object[]{3, "IBM", 100, 200}));
        inputHandlerStockQuoteStream.sendEvent(new EventImpl("StockQuoteStream", new Object[]{4, "IBM", 500, 200}));
        inputHandlerStockQuoteStream.sendEvent(new EventImpl("StockQuoteStream", new Object[]{5, "IBM", 470, 300}));
        inputHandlerStockQuoteStream.sendEvent(new EventImpl("StockQuoteStream", new Object[]{6, "IBM", 490, 300}));
        inputHandlerStockQuoteStream.sendEvent(new EventImpl("StockQuoteStream", new Object[]{7, "IBM", 500, 200}));
        inputHandlerStockQuoteStream.sendEvent(new EventImpl("StockQuoteStream", new Object[]{8, "IBM", 490, 300}));
    }
    ...
    private CallbackHandler assignCallback() {
        return new CallbackHandler("StockQuote") {
            public void callBack(Event event) {
                ...
                if ((Integer) event.getNthAttribute(1) == 100 && i == 0) {
                    eventCaptured = true;
                } else if ((Integer) event.getNthAttribute(1) == 490 & i == 1) {
                    eventCaptured = true;
                } else {
                    Assert.fail();
                }
