...
public class Sequence5TestCase {

    private static final Logger log = Logger.getLogger(Sequence5TestCase.class);
    private volatile int i;
    private volatile boolean eventCaptured;

    @Before
    public void info() {
        log.debug("-----Query processed: Testing repleting sequence with star, staring with non accepting event-----");
        eventCaptured = false;
        i = 0;
    }
    ...
    public void testAPI() throws SiddhiException, ProcessorInitializationException,
        ...
        Query query = qf.createQuery(
                "StockQuote",
                qf.output("priceA=$0.price", "priceB=$1.last.price", "priceC=$2.price"),
