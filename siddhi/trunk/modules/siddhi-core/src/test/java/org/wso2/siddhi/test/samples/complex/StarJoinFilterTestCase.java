...
public class StarJoinFilterTestCase {

    private static final Logger log = Logger.getLogger(StarJoinFilterTestCase.class);
    ...
    public static void main(String[] args)
            throws InvalidQueryException, ProcessorInitializationException, InterruptedException,
                   SiddhiException {
        // Test code
        StarJoinFilterTestCase simSimpleFilterTestCase = new StarJoinFilterTestCase();
    ...
    public void info() {
        if (log.isInfoEnabled()) {
            log.debug("-----Query processed: Running Star Join and filtering its output-----");
        }
    }

    @Test
    public void testCase() throws ProcessorInitializationException, SiddhiException,
...
//                qf.output("method=confirmation.payMethod", "rooms=request.noOfRooms"),
                ...
                qf.innerJoin(qf.from(reserveConfirmationEventStream).setWindow(WindowType.LENGTH, 50),
                             qf.from(requestEventStream).setWindow(WindowType.LENGTH, 60)),
...
//                    qf.output("Method=bought.method", "Rooms=bought.rooms"),
                //Due to star the attribute names are specified will be different.
                // ex: "confirmation_payMethod" corresponds to "payMethod" attribute of "confirmation" stream
                qf.output("Method=bought.confirmation_payMethod", "Rooms=bought.request_noOfRooms"),
                qf.from(query1),
//                    qf.condition("bought.rooms", GREATERTHAN_EQUAL, "2")
                qf.condition("bought.request_noOfRooms", GREATERTHAN_EQUAL, "2")
        );
