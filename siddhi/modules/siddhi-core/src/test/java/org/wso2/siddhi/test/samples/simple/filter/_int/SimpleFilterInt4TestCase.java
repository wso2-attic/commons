...
public class SimpleFilterInt4TestCase extends SimpleFilterBaseTestCase {

    private final Logger log = Logger.getLogger(this.getClass());


    protected SimpleCondition createAPICondition(QueryFactory qf) {
        return qf.condition("CSEStream.price+CSEStream.price*2", EQUAL, "(1+1)*3");
    }

    @Override
    protected String createQueryCondition() {
        return  "price+price*2 ==(1+1)*3";
    }

    protected void assertEvent(Event event) {
        Assert.assertEquals(event.getNthAttribute(0), 2);
    }
