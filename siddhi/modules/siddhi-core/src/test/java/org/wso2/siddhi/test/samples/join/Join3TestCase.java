...
public class Join3TestCase {

    private static final Logger log = Logger.getLogger(Join3TestCase.class);
    private volatile boolean eventCaptured = false;

    @Before
    public void info() {
        log.debug("-----Query processed: Testing aggregation on join with time window -----");
        eventCaptured = false;
    }
    ...
    public void testQuery() throws SiddhiException,
        ...
        siddhiManager.addConfigurations("confirmation:=timeStamp[long], memberId[int], payMethod[string];" +
                                        ...
                                        "bought:=select id=confirmation.memberId, payMethod=confirmation.payMethod, totalRooms=sum(request.noOfRooms), rooms=request.noOfRooms from confirmation[win.length=50],request[win.time=1000] where confirmation.memberId==request.memberId;");
    ...
    private CallbackHandler addCallback() {
        return new CallbackHandler("bought") {
            public void callBack(Event event) {
                ...
                if ((Integer) event.getNthAttribute(2) == 18) {
                    eventCaptured = true;
                }
            }
        };
