...
public class BpelRuntimeContextImpl implements BpelRuntimeContext {
    ...
    public void select(PickResponseChannel pickResponseChannel, Date timeout, boolean createInstance,
        ...
        for (int i = 0; i < selectors.length; ++i) {
            ...
            selector.correlationKeySet.add(new CorrelationKey("tenant", new String[]{_bpelProcess.getConf().getDeployer()}));
    ...
    public String invoke(int aid, PartnerLinkInstance partnerLink, Operation operation, Element outgoingMessage,
        ...
        if (getConfigForPartnerLink(partnerLink.partnerLink).usePeer2Peer && partnerEndpoint != null)
            p2pProcesses = _bpelProcess.getEngine().route(partnerEndpoint.serviceName, mex.getRequest(), _bpelProcess._pconf.getDeployer());
        ...
        if (p2pProcesses != null && !p2pProcesses.isEmpty()) {
            ...
            MyRoleMessageExchange myRoleMex = _bpelProcess.getEngine().createMessageExchange(
                    ...
                    operation.getName(), mex.getMessageExchangeId(), _bpelProcess._pconf.getDeployer());
     ...
     * @param scopeFrame Scope Frame
     * @param initData Data
     * @return Written variable value
     * @throws FaultException FaultException
     */
    public Node initializeVariable(VariableInstance variable, ScopeFrame scopeFrame, Node initData) throws FaultException {
        if (variable.declaration.extVar != null) /* external variable */ {
            if (__log.isDebugEnabled())
                __log.debug("Initialize external variable: name=" + variable.declaration + " value="+DOMUtils.domToString(initData));

            Node reference;
            ValueReferencePair writtenVal;
            //TODO need to test
            reference = fetchVariableData(variable, scopeFrame, true);
            try {
                writtenVal = writeExtVar(variable.declaration, reference, initData);
            } catch (ExternalVariableModuleException e) {
                throw new FaultException(_bpelProcess.getOProcess().constants.qnUnknownFault, e.getMessage(), e);
            }
            return writtenVal.value;
        } else /* normal variable */ {
            if (__log.isDebugEnabled())
            __log.debug("Initialize variable: name=" + variable.declaration +
                    " value=" + DOMUtils.domToString(initData));
            return writeVariable(variable, initData);
        }
    }
    ...
    public Node fetchVariableData(VariableInstance variable, ScopeFrame scopeFrame, boolean forWriting) throws FaultException {
        if (variable.declaration.extVar != null) {
            ...
            Node data = readVariable(variable.getScopeId(), variable.getName(), forWriting);
            if (data == null) {
    ...
    public OProcess getProcessModel() {
        return _bpelProcess.getOProcess();
    }

    public void registerForExtensionNotification(String notificationChannelId,
