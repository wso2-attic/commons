...
typedef struct svc_simulator_256b_config_rec
{
    char *filepath;
} svc_simulator_256b_config_rec_t;

typedef struct svc_simulator_256b_worker
{
    char *filepath;
    char *bodystring;
    int bodystring_len;
} svc_simulator_256b_worker_t;

svc_simulator_256b_worker_t *simulator_worker = NULL;
...
svc_simulator_256b_worker_t *
svc_simulator_256b_worker_create(
    apr_pool_t * p,
    char * filepath);
...
svc_simulator_256b_create_svr(
    ...
    server_rec * s)
{
    svc_simulator_256b_config_rec_t *conf = apr_palloc(p, sizeof(*conf));
    conf->filepath = NULL;
    return conf;
}
    ...
    server_rec * svr_rec)
{
    ...
    if(status)
    {
        ap_log_error(APLOG_MARK, APLOG_EMERG, status, svr_rec,
            "[Axis2] Error allocating mod_svc_simulator_256b memory pool");
        exit(APEXIT_CHILDFATAL);
    }

    simulator_worker = svc_simulator_256b_worker_create(pool, conf->filepath);
    if(!simulator_worker)
    {
        ap_log_error(APLOG_MARK, APLOG_EMERG, APR_EGENERAL, svr_rec,
            "[svc_simulator_256b] Error creating mod_svc_simulator_256b worker;filepath:%s", conf->filepath);
        exit(APEXIT_CHILDFATAL);
    }
}
...
svc_simulator_256b_worker_create(
    ...
    char * filepath)
{
    svc_simulator_256b_worker_t *worker = NULL;
    worker = (svc_simulator_256b_worker_t *) apr_palloc(pool, sizeof(svc_simulator_256b_worker_t));
    ...
    worker->bodystring = svc_simulator_256b_get_reply(pool);
    ...
    apr_pool_t * pool)
{
    ...
    bodystring = "<soapenv:Envelope xmlns:soapenv=\"http://www.w3.org/2003/05/soap-envelope\"><soapenv:Body><ns1:buyStocks xmlns:ns1=\"http://po.services.core.carbon.wso2.org\"><ns1:order><ns1:symbol>IBM</ns1:symbol><ns1:buyerID>asankha</ns1:buyerID><ns1:price>140.34</ns1:price><ns1:volume>200000</ns1:volume></ns1:order></ns1:buyStocks></soapenv:Body></soapenv:Envelope>";
