...
<!--
  ...
  2006-11-07: (JM) code review and cleanup + linked binding operations, added support for xs:import, wsdl:import
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
	...
	-->
	<xsl:param name="xsd-schema-location" select="'http://wso2.org/repos/wso2/trunk/commons/annotated-stylesheets/XMLSchema.xsd'"/>
	...
	<xsl:variable name="schemaTargetNamespace" select="wsdl:definitions/wsdl:types/xs:schema/@targetNamespace"/>
	...
	<xsl:template match="/">
		<html>
			<head>
				<title>WSDL 1.1 for <xsl:value-of select="wsdl:definitions/@targetNamespace"/></title>
				<style type="text/css">
...
.service     {background-color:rgb(240,197,166); text-align:right; border: 1px dashed black; padding:.5em} 
    ...
    <xsl:template match="wsdl:definitions/wsdl:types/xs:schema">
        <tr>
	...
	<xsl:template match="wsdl:input/@message | wsdl:output/@message | wsdl:port/@binding | wsdl:binding/@type">
		<xsl:call-template name="attribute">
			<xsl:with-param name="value-class">markup-name-attribute-value</xsl:with-param>
			<xsl:with-param name="reference">
				<xsl:call-template name="external-wsdl-reference"/>
	...
	<xsl:template name="additional-references">
		...
		<xsl:variable name="referenced-by" select="/wsdl:definitions/wsdl:message/wsdl:part[substring-after(@element,':') = $target]"/>
		...
		<xsl:choose>
			<xsl:when test="$supports-namespace-axis">
				<xsl:if test="not(../namespace::*[. = $targetNamespace and name(.) =
					...
					<xsl:value-of select="/wsdl:definitions/wsdl:import[@namespace=$uri]/@location"/>
				</xsl:if>
			</xsl:when>
			<xsl:otherwise>
				<xsl:choose>
			...
			<xsl:when test="$supports-namespace-axis">
				<xsl:if test="not(../namespace::*[. = $schemaTargetNamespace and name(.) =
					...
					<xsl:choose>
						...
						<xsl:otherwise>
							<xsl:value-of select="/wsdl:definitions/wsdl:types/xs:schema/xs:import[@namespace=$uri]/@schemaLocation"/>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:if>
			</xsl:when>
