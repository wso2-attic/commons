...
<!--
  ...
  2007-04-18: (JM) updated to new WSDL 2.0 namespace
  2006-11-08: (JM) created
-->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
	...
	-->
	<xsl:param name="xsd-schema-location" select="'XMLSchema.xsd'"/>
	...
	<xsl:variable name="schemaTargetNamespace" select="wsdl:description/wsdl:types/xs:schema/@targetNamespace"/>
	...
	<xsl:template match="/">
		<html>
			<head>
				<title>WSDL 2.0 for <xsl:value-of select="wsdl:description/@targetNamespace"/></title>
				<style type="text/css">
...
.extension-top-level {padding-left: 6em; text-indent:-5em}
.extension-second-level {padding-left: 7em; text-indent:-5em}
...
.service     {background-color:rgb(240,197,166); text-align:right; border: 1px dashed black; padding:.5em} 
	...
	<xsl:template match="wsdl:description/wsdl:*">
		...
		<tr>
			<xsl:choose>
				<xsl:when test="not(self::wsdl:documentation or self::wsdl:import or self::wsdl:include)">
					<td class="annotation-area">
	...
	<xsl:template match="wsdl:description/wsdl:types/*">
		<tr>
			<td colspan="2" class="annotation-area"/>
			<td class="source-area">
				<div class="extension-second-level">
					<xsl:call-template name="element-start">
                        <xsl:with-param name="class" select="'markup-extension-element'"/>
                    </xsl:call-template>
                </div>
    ...
    <xsl:template match="wsdl:description/wsdl:types/xs:schema | wsdl:description/wsdl:types/xs:import">
        <tr>
	...
	-->
	<xsl:template match="wsdl:binding/wsdl:operation/@name | wsdl:binding/wsdl:fault/@name" priority="1">
	...
	<xsl:template match="wsdl:input/@element[not(contains(.,'#'))] | wsdl:output/@element[not(contains(.,'#'))] | wsdl:fault/@element[not(contains(.,'#'))]">
		<xsl:call-template name="attribute">
			<xsl:with-param name="value-class">markup-name-attribute-value</xsl:with-param>
			<xsl:with-param name="reference">
	...
	<xsl:template match="wsdl:service/@interface">
		<xsl:call-template name="attribute">
			<xsl:with-param name="value-class">markup-name-attribute-value</xsl:with-param>
			<xsl:with-param name="reference">
				<xsl:call-template name="external-schema-reference"/>
	...
	<xsl:template match="wsdl:interface/@extends">
		<xsl:call-template name="attribute">
			<xsl:with-param name="value-class">markup-name-attribute-value</xsl:with-param>
			<xsl:with-param name="preconstructed-reference">
	...
	<xsl:template name="reference-list">
		<xsl:param name="list"/>
		<xsl:variable name="first-reference" select="substring-before($list,' ')"/>
		<xsl:variable name="other-references" select="substring-after($list,' ')"/>
		<xsl:variable name="reference-url">
			<xsl:call-template name="external-schema-reference"/>
			<xsl:text>#_interface_</xsl:text>
			<xsl:value-of select="substring-after($first-reference,':')"/>
		</xsl:variable>
		<a href="{$reference-url}"><xsl:value-of select="$first-reference"/></a>
		<xsl:if test="$other-references">
			<xsl:text> </xsl:text>
			<xsl:call-template name="reference-list">
				<xsl:with-param name="list" select="$other-references"/>
			</xsl:call-template>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template name="additional-references">
		...
		<xsl:variable name="referenced-by" select="/wsdl:description/wsdl:interface/wsdl:operation/wsdl:*[@element = $target or substring-after(@element,':') = $target]"/>
		...
		<xsl:choose>
			<xsl:when test="$supports-namespace-axis">
				<xsl:if test="not(../namespace::*[. = $targetNamespace and name(.) =
					...
					<xsl:value-of select="/wsdl:description/wsdl:import[@namespace=$uri]/@location"/>
				</xsl:if>
			</xsl:when>
			<xsl:otherwise>
				<xsl:choose>
					<xsl:when test="substring-before(current(),':')= 'xs' or substring-before(current(),':')= 'xsd'">
			...
			<xsl:when test="$supports-namespace-axis">
				<xsl:if test="not(../namespace::*[. = $schemaTargetNamespace and name(.) =
					...
					<xsl:choose>
						...
						<xsl:otherwise>
							<xsl:value-of select="/wsdl:description/wsdl:types/xs:import[@namespace=$uri]/@schemaLocation"/>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:if>
			</xsl:when>
