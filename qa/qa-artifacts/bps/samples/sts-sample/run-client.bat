...
rem ---------------------------------------------------------------------------
rem Client script for the STS Sample 
rem
...
cd %WSO2WSAS_HOME%
set CLIENT_CLASSPATH=.\samples\sts-sample\conf;.\samples\sts-sample\temp\classes
FOR %%C in (.\repository\lib\*.jar) DO set CLIENT_CLASSPATH=!CLIENT_CLASSPATH!;.\repository\lib\%%~nC%%~xC
...
%_RUNJAVA% %JAVA_OPTS% -Dwso2wsas.home="%WSO2WSAS_HOME%" -cp "%CLIENT_CLASSPATH%" -Djava.endorsed.dirs="%WSO2WSAS_HOME%\lib\endorsed";"%JAVA_HOME%\jre\lib\endorsed";"%JAVA_HOME%\lib\endorsed" org.wso2.wsas.sample.sts.client.Client %*
cd %CURRENT_DIR%
endlocal
:end
