...
public class ReScheduleTaskTest extends TestTemplateRSS {
    ...
    public void runSuccessCase() {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            Assert.assertEquals("Task not properly Stopped after rescheduling task", empSalary, currentSalary);
            try {
                Thread.sleep(5000);
            } catch (InterruptedException e) {
                log.error("InterruptedException :", e);
        ...
        while(startTime.getTimeInMillis() >= Calendar.getInstance().getTimeInMillis()){
            try {
                Thread.sleep(1000);
            } catch (InterruptedException e) {
                log.error("InterruptedException :", e);
        ...
        for (int i = 0; i < 10; i++) {
            ...
            Assert.assertEquals("Task not properly rescheduled", (empSalary = empSalary + 10000), currentSalary);
            try {
                Thread.sleep(5000);
            } catch (InterruptedException e) {
                log.error("InterruptedException :", e);
