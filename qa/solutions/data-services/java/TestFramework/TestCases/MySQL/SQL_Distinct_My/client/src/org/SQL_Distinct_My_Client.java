...
public class SQL_Distinct_My_Client {
	...
	public static void main(String[] args)throws org.apache.axis2.AxisFault 
	{
		...
		opts.setAction("urn:OP_Distinct"); // soap action field copied from the wsdl file
		sc.setOptions(opts);
		
		OMElement result = sc.sendReceive(CreatePayload());
		System.out.println(result);
/*		OMElement firstChild = (OMElement)result.getFirstOMChild(); 
		OMElement secondChild = (OMElement)firstChild.getFirstOMChild();
		System.out.println(secondChild.getText());
*/
	}
	...
	public static OMElement CreatePayload() // this function create a message to send to server
	{
		...
		OMElement OP1 = fac.createOMElement("OP_Distinct", omns); // Operation name
		return OP1;
	}
}
