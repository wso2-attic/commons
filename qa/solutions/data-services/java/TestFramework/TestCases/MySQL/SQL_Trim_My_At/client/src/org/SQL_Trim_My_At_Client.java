...
public class SQL_Trim_My_At_Client {
	...
	public static void main(String[] args)throws org.apache.axis2.AxisFault 
	{
		...
		opts.setAction("urn:OP_LTrim"); // soap action field copied from the wsdl file
		sc.setOptions(opts);
		OMElement result = sc.sendReceive(CreatePayload());
		System.out.println(result);
		
		opts.setAction("urn:OP_Trim");
		sc.setOptions(opts);
		OMElement Result2 = sc.sendReceive(CreatePayload2());
		System.out.println(Result2);
		
		opts.setAction("urn:OP_RTrim");
		sc.setOptions(opts);
	...
	public static OMElement CreatePayload() // this function create a message to send to server
	{
		...
		OMElement OP1 = fac.createOMElement("OP_LTrim", omns); // Operation name
		return OP1;
	}
	...
	public static OMElement CreatePayload2() 
	{
		...
		OMElement OP2 = MyFac.createOMElement("OP_Trim", MyName);
		return OP2;
	...
	{
		OMFactory MyFac = OMAbstractFactory.getOMFactory();
		OMNamespace MyName = MyFac.createOMNamespace("http://ws.wso2.org/dataservice", "f");
		OMElement OP3 = MyFac.createOMElement("OP_RTrim", MyName);
		return OP3;
