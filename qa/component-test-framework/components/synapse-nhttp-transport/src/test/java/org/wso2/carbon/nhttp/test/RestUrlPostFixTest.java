...
public class RestUrlPostFixTest extends TestTemplate {
    private static final Log log = LogFactory.getLog(RestUrlPostFixTest.class);
    String clientLog = "";

    @Override
    public void init() {
        log.info("Initializing RestUrlPostFix Tests");
        log.debug("RestUrlPostFix Tests Initialised");
    }

    @Override
    public void runSuccessCase() {
        log.debug("Running RestUrlPostFix SuccessCase ");
        ...
        try {
            ...
            String xmlPath = frameworkPath + File.separator + "components"
                    + File.separator + "synapse-nhttp-transport" + File.separator + "src" + File.separator + "test" + File.separator + "resources" + File.separator + "rest_url_postfix.xml";
            ...
            clientLog = Axis2Client.fireClient("ant stockquote -Dtrpurl=http://" + HOST_NAME + ":" + HTTP_PORT + "/services/SimpleStockQuoteService");

            System.out.println(clientLog);

            Assert.assertTrue(clientLog.contains("Standard :: Stock price"));
            log.info("Running RestUrlPostFix success case");
            log.debug("Running RestUrlPostFix success case");
            clientLog = "";
        }

        catch (Exception e) {
            Assert.fail("RestUrlPostFix Test Failed : " + e);
            log.error("RestUrlPostFix Test Failed : " + e.getMessage());
        }
    }
    ...
    public void runFailureCase() {
        ...
        log.info("Running RestUrlPostFix failure case");
        log.debug("Running RestUrlPostFix  failure case");
    }
