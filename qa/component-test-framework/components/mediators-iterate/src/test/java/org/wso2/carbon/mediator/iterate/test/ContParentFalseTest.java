package org.wso2.carbon.mediator.iterate.test;
...
// Test iterate mediator with continue parent false

public class ContParentFalseTest extends TestTemplate {

    private static final Log log = LogFactory.getLog(ContParentFalseTest.class);

    @Override
    public void init() {
        log.info("Initializing Iterate Mediator Test class ");
        log.debug("Iterate Mediators Test Initialized");
    }
    ...
    public void runSuccessCase() {
        ...
        try {
            ...
            String xmlPath = frameworkPath + File.separator + "components" + File.separator + "mediators-iterate"
                             + File.separator + "src" + File.separator + "test" + File.separator + "resources" + File.separator + "iterate.xml";
            ...
            if (!result.toString().contains("IBM")) {
                log.error("Iterate Mediator does not work");
                Assert.fail("Iterate Mediator does not work");
            }
        }
        catch (Exception e) {
            e.printStackTrace();
            log.error("Iterate mediator doesn't work : " + e.getMessage());
