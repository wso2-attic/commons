...
public class Sample9 extends SampleTestTemplate {
    ...
    protected void invokeClient() {
        System.out.println(HOST_NAME + ":" + BACKENDSERVER_HTTP_PORT);
    ...
    protected void runSuccessTest() {
        ...
        log.info("running sample9 success case");
        log.debug("running sample9 success case");
    }

    @Override
    protected void runFailureTest() {
        Assert.assertFalse(clientLog.contains("Standard :: Stock price"));
        log.info("running sample9 failure case");
        log.debug("running sample9 failure case");
    }
}
