...
public class Sample50 extends SampleTestTemplate {
    ...
    protected void invokeClient() {
        clientLog = Axis2Client.fireClient("ant stockquote -Dtrpurl=http://" + HOST_NAME + ":" + HTTP_PORT + "/services/StockQuote -Drest=true");
    ...
    protected void runSuccessTest() {
        ...
        log.info("running sample50 success case");
        log.debug("running sample50 success case");
    }

    @Override
    protected void runFailureTest() {
        Assert.assertFalse(clientLog.contains("Standard :: Stock price"));
        log.info("running sample50 failure case");
        log.debug("running sample50 failure case");
    }
