...
public class Sample202 extends SampleTestTemplate {
    private static final Log log = LogFactory.getLog(Sample202.class);
    ...
    protected void invokeClient() {
        clientLog = Axis2Client.fireClient("ant stockquote -Dsymbol=IBM -Dmode=quote -Daddurl=http://" + HOST_NAME + ":" + HTTP_PORT + "/services/StockQuoteProxy -Dwsrm11=true");
    }

    @Override
    protected void runSuccessTest() {
        Assert.assertTrue(clientLog.contains("Standard :: Stock price ="));
        log.info("running sample202 success case");
        log.debug("running sample202 success case");
    }

    @Override
    protected void runFailureTest() {
        Assert.assertFalse(clientLog.contains("Standard :: Stock price ="));
        log.info("running sample202 failure case");
        log.debug("running sample202 failure case");
    }
