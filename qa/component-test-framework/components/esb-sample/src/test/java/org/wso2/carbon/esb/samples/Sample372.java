...
public class Sample372 extends SampleTestTemplate {
    ...
    protected void invokeClient() {
        clientLog = Axis2Client.fireClient("ant stockquote -Daddurl=http://" + HOST_NAME + ":" + HTTP_PORT + "/services/StockQuoteProxy");
    ...
    protected void runSuccessTest() {
        ...
        log.info("running sample372 success case");
        log.debug("running sample372 success case");
    }

    @Override
    protected void runFailureTest() {
        Assert.assertFalse(clientLog.contains("Standard :: Stock price"));
        log.info("running sample372 failure case");
        log.debug("running sample372 failure case");
    }
}
