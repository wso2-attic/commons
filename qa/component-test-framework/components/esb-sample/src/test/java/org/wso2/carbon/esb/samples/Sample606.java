...
public class Sample606 extends SampleTestTemplate {
    private static final Log log = LogFactory.getLog(Sample606.class);
    ...
    protected void invokeClient() {
        clientLog = Axis2Client.fireClient("ant stockquote -Daddurl=http://localhost:9000/services/SimpleStockQuoteService -Dtrpurl=http://localhost:8280/ -Dmode=customquote");
    ...
    protected void runSuccessTest() {
        ...
        log.info("running sample606 success case");
        log.debug("running sample606 success case");
    }

    @Override
    protected void runFailureTest() {
        Assert.assertFalse(clientLog.contains("Custom :: Stock price"));
        log.info("running sample606 failure case");
        log.debug("running sample606 failure case");
    }
