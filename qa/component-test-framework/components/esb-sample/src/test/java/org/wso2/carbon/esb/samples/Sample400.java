...
public class Sample400 extends SampleTestTemplate {
    private static final Log log = LogFactory.getLog(Sample400.class);
    ...
    protected void invokeClient() {
        clientLog = Axis2Client.fireClient("ant stockquote -Daddurl=http://" + HOST_NAME + ":" + HTTP_PORT + "/services/SplitAggregateProxy -Ditr=4");
    ...
    protected void runSuccessTest() {
        ...
        log.info("running sample400 success case");
        log.debug("running sample400 success case");
    }

    @Override
    protected void runFailureTest() {
        Assert.assertFalse(clientLog.contains("Standard :: Stock price"));
        log.info("running sample400 failure case");
        log.debug("running sample400 failure case");
    }
}
