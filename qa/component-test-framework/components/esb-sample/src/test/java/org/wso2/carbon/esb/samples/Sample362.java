...
// dbreport and dblookup mediators together sample
public class Sample362 extends SampleTestTemplate {
    ...
    protected void invokeClient() {
        clientLog = Axis2Client.fireClient("ant stockquote -Daddurl=http://" + HOST_NAME + ":" + BACKENDSERVER_HTTP_PORT + "/SimpleStockQuoteService -Dtrpurl=http://" + HOST_NAME + ":" + HTTP_PORT + "/ -Dsymbol=IBM");
    }

    @Override
    protected void runSuccessTest() {
        Assert.assertTrue(clientLog.contains("Standard :: Stock price"));
        log.info("running sample362 success case");
        log.debug("running sample362 success case");
    ...
    protected void runFailureTest() {
        ...
        log.info("running sample362 failure case");
        log.debug("running sample362 failure case");
