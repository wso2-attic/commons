...
public class SpringMediatorTest extends TestTemplate {

    private static final Log log = LogFactory.getLog(SpringMediatorTest.class);

    @Override
    public void init() {
        log.info("Initializing Spring Mediator Test class ");
        log.debug("Spring Mediators Test Initialized");
    }
    ...
    public void runSuccessCase() {
        ...
        try {
            ...
            String xmlPath = frameworkPath + File.separator + "components" + File.separator + "mediators-spring"
                             + File.separator + "src" + File.separator + "test" + File.separator + "resources" + File.separator + "SpringTestConfig.xml";
            ...
            if (!result.toString().contains("MSFT")) {
                log.error("Spring Mediator does not work");
                Assert.fail("Spring Mediator does not work");
            }
        }
        catch (Exception e) {
            e.printStackTrace();
            log.error("spring mediator doesn't work : " + e.getMessage());
