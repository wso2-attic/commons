...
public class CalloutMediatorTest extends TestTemplate {
    private static final Log log = LogFactory.getLog(CalloutMediatorTest.class);

    @Override
    public void init() {
        log.info("Initializing Callout Mediator Tests");
        log.debug("Callout mediator Tests Initialised");
    }

    @Override
    public void runSuccessCase() {
        log.debug("Running Callout mediator SuccessCase ");
        ...
        try {
            ...
            String xmlPath = frameworkPath + File.separator + "components" + File.separator + "mediators-callout"
                             + File.separator + "src" + File.separator + "test" + File.separator + "resources" + File.separator + "callout.xml";
            ...
            if (!result.toString().contains("IBM")) {
                Assert.fail("Callout Mediator not invoked");
                log.error("Callout Mediator not invoked");
            }
        }
        catch (Exception e) {
            e.printStackTrace();
            log.error("Callout Mediator doesn't work : " + e.getMessage());
