package org.wso2.carbon.mediator.property.test.getProperty.synapseScope;
...
public class GetPropertySynapseTest extends TestTemplate {
    private static final Log log = LogFactory.getLog(GetPropertySynapseTest.class);

    @Override
    public void init() {
        log.info("Initializing Property Mediator getProperty Tests");
        log.debug("Property Mediator getProperty Tests Initialised");
    }

    @Override
    public void runSuccessCase() {
        log.debug("Property Mediator getProperty SuccessCase ");
        ...
        try {
            ...
            String xmlPath = frameworkPath + File.separator + "mediators-property"
                    + File.separator + "src" + File.separator + "test" + File.separator + "resources" + File.separator + "getProperty" + File.separator + "synapseScope" + File.separator + "synapse.xml";
            ...
            if (!result.toString().contains("IBM")) {
                Assert.fail();
                log.error("Property Mediator getProperty Test Failed");
            }
        }
        catch (Exception e) {
            log.error("Property Mediator getProperty doesn't work : " + e.getMessage());
