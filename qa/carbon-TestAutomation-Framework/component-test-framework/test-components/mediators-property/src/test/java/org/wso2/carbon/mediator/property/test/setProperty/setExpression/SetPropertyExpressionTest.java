package org.wso2.carbon.mediator.property.test.setProperty.setExpression;
...
public class SetPropertyExpressionTest extends TestTemplate {
    private static final Log log = LogFactory.getLog(SetPropertyExpressionTest.class);

    @Override
    public void init() {
        log.info("Initializing Property Mediator setExpression Tests");
        log.debug("Property Mediator setExpression Tests Initialised");
    }

    @Override
    public void runSuccessCase() {
        log.debug("Property Mediator setExpression SuccessCase ");
        ...
        try {
            ...
            String xmlPath = frameworkPath + File.separator + "mediators-property"
                    + File.separator + "src" + File.separator + "test" + File.separator + "resources" + File.separator + "setProperty" + File.separator + "setExpression" + File.separator + "synapse.xml";
            ...
            if (!result.toString().contains("IBM")) {
                Assert.fail();
                log.error("Property Mediator setExpression Test Failed");
            }
        }
        catch (Exception e) {
            log.error("Property Mediator setExpression doesn't work : " + e.getMessage());
