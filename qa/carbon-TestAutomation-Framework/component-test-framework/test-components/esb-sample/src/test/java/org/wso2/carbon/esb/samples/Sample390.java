...
public class Sample390 extends SampleTestTemplate {
    ...
    protected void invokeClient() {
        clientLog = Axis2Client.fireClient("ant stockquote -Daddurl=http://" + HOST_NAME + ":" + HTTP_PORT + "/services/StockQuoteProxy -Dmode=customquote");
    ...
    protected void runSuccessTest() {
        ...
        log.info("running sample390 success case");
        log.debug("running sample390 success case");
    }

    @Override
    protected void runFailureTest() {
        Assert.assertFalse(clientLog.contains("Custom :: Stock price"));
         log.info("running sample390 failure case");
        log.debug("running sample390 failure case");
    }
}
