...
//Mediation Sample
public class Sample600 extends SampleTestTemplate {
    private static final Log log = LogFactory.getLog(Sample600.class);
    ...
    protected void runSuccessTest() {
        ...
        log.info("running sample600 success case");
        log.debug("running sample600 success case");
    }

    @Override
    protected void runFailureTest() {
        Assert.assertFalse(clientLog.contains("Standard :: Stock price"));
        log.info("running sample600 failure case");
        log.debug("running sample600 failure case");
    }
}
