...
public class Sample201 extends SampleTestTemplate {
    private static final Log log = LogFactory.getLog(Sample201.class);
    ...
    protected void invokeClient() {
        clientLog = Axis2Client.fireClient("ant stockquote -Dsymbol=IBM -Dmode=quote -Daddurl=http://" + HOST_NAME + ":" + HTTP_PORT + "/services/StockQuoteProxy -Dwsrm=true");
    }

    @Override
    protected void runSuccessTest() {
        Assert.assertTrue(clientLog.contains("Standard :: Stock price ="));
        log.info("running sample201 success case");
        log.debug("running sample201 success case");
    }

    @Override
    protected void runFailureTest() {
        Assert.assertFalse(clientLog.contains("Standard :: Stock price ="));
        log.info("running sample201 failure case");
        log.debug("running sample201 failure case");
    }
