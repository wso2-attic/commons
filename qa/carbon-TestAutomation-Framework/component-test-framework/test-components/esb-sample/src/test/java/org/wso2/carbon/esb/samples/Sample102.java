...
public class Sample102 extends SampleTestTemplate {
    private static final Log log = LogFactory.getLog(Sample102.class);
    ...
    protected void invokeClient() {
        clientLog = Axis2Client.fireClient("ant stockquote -Dsymbol=IBM -Dmode=quote -Daddurl=http://" + HOST_NAME + ":" + HTTP_PORT);
    ...
    protected void runSuccessTest() {
        ...
        log.info("running sample102 success case");
        log.debug("running sample102 success case");
    }

    @Override
    protected void runFailureTest() {
        Assert.assertFalse(clientLog.contains("Standard :: Stock price ="));
        log.info("running sample102 failure case");
        log.debug("running sample102 failure case");
    }
