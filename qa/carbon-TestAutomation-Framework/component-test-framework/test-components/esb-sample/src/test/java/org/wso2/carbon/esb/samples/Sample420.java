...
public class Sample420 extends SampleTestTemplate {
    ...
    protected void invokeClient() {
        clientLog = Axis2Client.fireClient("ant stockquote -Dtrpurl=http://" + HOST_NAME + ":" + HTTP_PORT + "/");
    }

    @Override
    protected void runSuccessTest() {
        Assert.assertTrue(clientLog.contains("Standard :: Stock price"));
        log.info("running sample420 success case");
        log.debug("running sample420 success case");
    }

    @Override
    protected void runFailureTest() {
        Assert.assertFalse(clientLog.contains("Standard :: Stock price"));
        log.info("running sample420 failure case");
        log.debug("running sample420 failure case");
    }
}
