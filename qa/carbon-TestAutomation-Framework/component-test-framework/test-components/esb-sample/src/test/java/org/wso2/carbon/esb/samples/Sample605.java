...
//Rules Sample
public class Sample605 extends SampleTestTemplate {
    private static final Log log = LogFactory.getLog(Sample605.class);
    ...
    protected void runSuccessTest() {
        ...
        log.info("running sample605 success case");
        log.debug("running sample605 success case");
    }

    @Override
    protected void runFailureTest() {
        Assert.assertFalse(clientLog.contains("Standard :: Stock price"));
        log.info("running sample605 failure case");
        log.debug("running sample605 failure case");
    }
}
