...
public class LogMediatorSimpleLevelTest extends TestTemplate {
    private static final Log log = LogFactory.getLog(LogMediatorSimpleLevelTest.class);
    String search_l1 = "request$ inComing = ***Incoming Message***$";
    String search_l2 = "inExpression = Echo String - urn:getQuote";
    String search_l3 = "WSAction:";
    String search_l4 = "response$ outgoing = ***Outgoing Message***";

    @Override
    public void init() {
        log.info("Initializing Simple Level Log Mediator Tests");
        log.debug("Log Mediator Tests Initialised");
    }

    @Override
    public void runSuccessCase() {
        log.debug("Running Simple Level Log Mediator SuccessCase ");
        Calendar cal = Calendar.getInstance();
        OMElement result = null;
        boolean isFound_search_l1 = false;
        boolean isFound_search_l2 = false;
        boolean isFound_search_l3 = false;
        boolean isFound_search_l4 = false;
        ...
        try {
            ...
            String xmlPath = frameworkPath + File.separator + "mediators-log"
                             + File.separator + "src" + File.separator + "test" + File.separator + "resources" + File.separator + "simple" + File.separator + "synapse.xml";
            ...
            if (!result.toString().contains("IBM")) {
                Assert.fail();
                log.error("Simple Level Log Mediation Failed");
            ...
            for (int i = removeMessageLength; i <= logMessages.length - 1; i++) {
                ...
                if (logMessages[i].getLogMessage().contains(search_l4)) {
                    isFound_search_l4 = true;
                }
            }
            ...
            assertTrue("WSAction: $ SOAPAction: $ MessageID: not found in log message", isFound_search_l3);
            assertTrue("response$ outgoing = ***Outgoing Message*** not found in log message", isFound_search_l4);
        }
        catch (Exception e) {
            log.error("Simple Level Log Mediator doesn't work : " + e.getMessage());
