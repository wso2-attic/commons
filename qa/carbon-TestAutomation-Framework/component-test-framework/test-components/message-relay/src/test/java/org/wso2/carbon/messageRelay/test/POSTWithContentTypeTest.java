...
public class POSTWithContentTypeTest extends TestTemplate {
    ...
    @Override
    public void init() {
        log.info("Initializing WithContentType Message Relay Tests");
        log.debug("WithContentType Message Relay Tests Initialised");
    }

    @Override
    public void runSuccessCase() {
        log.debug("Running WithContentType Message Relay SuccessCase ");
            ...
            if (FrameworkSettings.STRATOS.equalsIgnoreCase("false")) {
                ...
                result = stockQuoteClient.stockQuoteClientforProxy(trpUrl, null, "IBM");
                    ...
                    if (!contentType.contains("application/xml; charset=UTF-8")) {
                        ...
                        Assert.fail("Message Relay with Content Type Test Failed");
                        log.error("Message Relay with Content Type Test Failed");
                    }

                }
                catch(MalformedURLException
                e1){
                    log.error("Message Relay WithContentType Test doesn't work : " + e1.getMessage());


                }
                catch(Exception
                e){
                    log.error("Message Relay WithContentType Test doesn't work : " + e.getMessage());
