...
public class InitializeResourceAdminCommand {

    private static final Log log = LogFactory.getLog(InitializeResourceAdminCommand.class);

    public ResourceAdminServiceStub executeAdminStub(String sessionCookie) {
        log.debug("sessionCookie:" + sessionCookie);
        String serviceURL = null;
        FrameworkSettings.getProperty();
        if(FrameworkSettings.STRATOS.equalsIgnoreCase("true")){
        serviceURL = "https://" + HOST_NAME + "/services/ResourceAdminService";
        }
        else if(FrameworkSettings.STRATOS.equalsIgnoreCase("false")){
           serviceURL = "https://" + HOST_NAME + ":" + HTTPS_PORT + "/services/ResourceAdminService";
        }
