...
sub invoke_tests
{
print "Invoking axis2c tests...\n";
    
    my @output;
    my $all = 0;
    my $success = 0;
    my $fail = 0;
    my @nodes = $root->getElementsByTagName("axis2c_tests");
    ...
    $logger->info ("Repeating results may occure for loop ahead \n\n");
     ...
     print "Unsuccesful Invokes     : $fail\n";
...
{
  my @nodes = $root->getElementsByTagName("axis2c_tests");
  $logger->info ("got children of \"axis2_tests\" by tag name $nodes[0]");
  my @test = $nodes[0]->getElementsByTagName("test");
  $logger->info ("got child nodes ");
  ...
  chdir "axis2c" or die "unable to change director ";
  
  $logger->info ("changed directory to $working[0]\\testcases\\axis2c ");
  $logger->info ("Repeating results may occure for loop ahead \n\n");
   foreach (@test)
    {
       my $name = $_->getElementsByTagName("name");
