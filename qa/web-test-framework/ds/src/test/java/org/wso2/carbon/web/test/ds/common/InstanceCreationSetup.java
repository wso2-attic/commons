...
public class InstanceCreationSetup extends TestCase {
    ...
    public Selenium openBrowser(String strUrl) throws Exception{
        ...
        browser = new DefaultSelenium("localhost", 4444, "*"+loadProperties().getProperty("browser.version"), "https://" + loadProperties().getProperty("host.name") + ":" + loadProperties().getProperty("http.port"));
        browser.start();
        browser.open(strUrl);
