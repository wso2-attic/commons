...
for i in $var; do
	...
	if test $# = 1;then
	      mvn -e clean install -DfirefoxProfileTemplate=$1 -Dtest.suite=$i
	fi
...
###################################################">> reports/surefire_report.txt
	cat target/surefire-reports/org.wso2.carbon.web.test.GaaS.AllTests.txt >> reports/surefire_report.txt


	egrep "Tests run:" target/surefire-reports/org.wso2.carbon.web.test.GaaS.AllTests.txt  > reports/tmp.txt
	...
	egrep "Tests run:"  target/surefire-reports/org.wso2.carbon.web.test.GaaS.AllTests.txt > reports/timeElapsedTemp.txt
