...
public class HelpPagesTest extends TestCase {
    ...
    public void testDashboardHelpPage() throws Exception {

        registryCommon.signOut();
        UmCommon.loginToUI(adminUserName, adminPassword);

        registryCommon.gotoDashboardPage();

        assertTrue(selenium.isTextPresent("Dashboard"));
        assertTrue(selenium.isTextPresent("Main Dashboard"));
        ...
        String dashboardHelp = selenium.getEval("{var windowId; for(var x in selenium.browserbot.openedWindows ) {windowId=x;} }");
        selenium.selectWindow(dashboardHelp);

        selenium.windowFocus();

        assertTrue(selenium.isTextPresent("The Dashboard"));
        assertTrue(selenium.isTextPresent("The Dashboard is an Executive Information System, " +
                "which gives users a Bird's Eye view of their SOA by way of a set of Gadgets. Users can use the " +
                "default set of Gadgets shipped with a WSO2 product as well as adding their own."));
        assertTrue(selenium.isElementPresent("//img[@alt='dashboard-main.png']"));
        assertTrue(selenium.isTextPresent("Gadgets"));
        assertTrue(selenium.isTextPresent("Tabs"));
        assertTrue(selenium.isTextPresent("Further Reading"));
        assertEquals("Gadget Developer's Guide", selenium.getText("link=Gadget Developer's Guide"));
        assertTrue(selenium.isTextPresent("Google Gadget Specification"));
    ...
    public void testMonitoredServerHelpPage() throws Exception {

        registryCommon.signOut();
        UmCommon.loginToUI(adminUserName, adminPassword);

        registryCommon.gotomonitoredServerPage();

        assertTrue(selenium.isTextPresent("Monitored Servers"));

        selenium.click("link=Help");

        String monitoredServerHelp = selenium.getEval("{var windowId; for(var x in selenium.browserbot.openedWindows ) {windowId=x;} }");
        selenium.selectWindow(monitoredServerHelp);

        selenium.windowFocus();

        assertTrue(selenium.isTextPresent("Configuring Monitored Servers"));
        assertTrue(selenium.isElementPresent("//img"));
        assertTrue(selenium.isTextPresent("Server Monitoring allows centralized statistics gathering. The gathered" +
                " statistics is presented through the server monitoring gadgets in the dashboard. The servers of which " +
                "the statistics should be collected has to be provided here.Once a server is added to this list, server" +
                " monitoring will be enabled. Only way to disable monitoring is to remove the server entry from the list."));
        assertTrue(selenium.isTextPresent("Removing a server"));
        assertTrue(selenium.isTextPresent("To remove a server from monitored server list, and to stop statistics collection, " +
                "click on the delete link next to the particular server in the monitored server list."));
