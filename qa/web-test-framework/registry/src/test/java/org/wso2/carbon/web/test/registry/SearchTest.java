...
public class SearchTest extends CommonSetup {
    ...
    public void testErrorSessionTimeout() throws Exception {
        registryCommon.signOut();
        UmCommon.loginToUI(adminUserName, adminPassword);
        selenium.click("link=Search");
        selenium.waitForPageToLoad("30000");
        Properties properties = new Properties();
        properties.load(new FileInputStream(".." + File.separator + "commons" + File.separator +
                "src" + File.separator + "test" + File.separator + "resources" + File.separator +
                "framework.properties"));
        String url = "https://" + properties.getProperty("host.name") + ":" +
                properties.getProperty("https.fe.port") + properties.getProperty("context.root");
        if (url.endsWith("/")) {
            url += "carbon/admin/logout_action.jsp";
        } else {
            url += "/carbon/admin/logout_action.jsp";
        }
        selenium.openWindow(url, UUID.randomUUID().toString());
        selenium.click("#_0");
        for (int second = 0; ; second++) {
            if (second >= 60) fail("timeout");
            try {
                if (selenium.isElementPresent("messagebox-error")) break;
            } catch (Exception e) {
            }
            Thread.sleep(1000);
        }
        assertEquals("Session timed out. Please login again.", selenium.getText("//div[@id='messagebox-error']/p"));
        selenium.click("//button[@type='button']");
        Thread.sleep(1000);
