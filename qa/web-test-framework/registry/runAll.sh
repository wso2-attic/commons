...
rm -r reports
mkdir reports
var=`cat registry_test_suites.txt`
...
for i in $var; do
	...
	echo " +++++++++++++++++++++profle $ffprofile "
	if test $# = 1;then
		mvn -e clean install -DfirefoxProfileTemplate=$ffprofile  -Dtest.suite=$i
	fi

	if echo "$@" | grep -q "[-o]"
	then
	      echo "offline mode"
	      mvn -e clean install -o  -DfirefoxProfileTemplate=$ffprofile -Dtest.suite=$i
	fi
...
###################################################">> reports/surefire_report.txt
	cat target/surefire-reports/org.wso2.carbon.web.test.registry.AllTests.txt >> reports/surefire_report.txt


	egrep "Tests run:" target/surefire-reports/org.wso2.carbon.web.test.registry.AllTests.txt  > reports/tmp.txt
	...
	egrep "Tests run:"  target/surefire-reports/org.wso2.carbon.web.test.registry.AllTests.txt > reports/timeElapsedTemp.txt
	cut -d " " -f12 reports/timeElapsedTemp.txt | cut -d "c" -f1 > reports/timeElapsed.txt
	current=$(sed 's/,//g' reports/timeElapsed.txt)
	totalt=$(echo "$current+$totalt" | bc)
	rm reports/temp.txt
done
...
echo $totalt > reports/timeElapsed.txt
total=$(sed -e :a -e 's/\(.*[0-9]\)\([0-9]\{3\}\)/\1,\2/;ta' reports/timeElapsed.txt)
...
echo "Total Time Elapsed: ${totalt} sec">> reports/surefire_report.txt
echo $totalt
echo "\n---------------------------------------------------">> reports/surefire_report.txt
