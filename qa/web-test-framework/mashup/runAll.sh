...
for i in $var; do
	...
	if test $1 = '-o';then
	      mvn -e clean install -o -DfirefoxProfileTemplate=$2 -Dtest.suite=$i
	fi
...
###################################################">> reports/surefire_report.txt
	cat target/surefire-reports/org.wso2.carbon.web.test.mashup.AllTests.txt >> reports/surefire_report.txt


	egrep "Tests run:"  target/surefire-reports/org.wso2.carbon.web.test.mashup.AllTests.txt > reports/tmp.txt
